...
def connect(path):
    ...
    db.executescript('''
        ...
        CREATE UNIQUE INDEX IF NOT EXISTS idx_invoices_customer_invoice
...
def insert_invoice(db, row):
    ...
    if old:
        if (round(float(old['amount']), 2) == round(float(row['amount']), 2) and
...
def insert_payment(db, row, invoice_id):
    ...
    if old:
        if (old['customer_id'] == row['customer_id'] and
                ...
                round(float(old['amount']), 2) == round(float(row['amount']), 2)):
            return 'skipped'
