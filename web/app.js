...
async function refresh() {
  ...
  if (customer) invoiceUrl += `&customer=${encodeURIComponent(customer)}`;
...
async function submitImport(form) {
  ...
  try {
    ...
    if (!response.ok) {
      feedback.textContent = `Import failed: ${data.error || response.statusText || 'Unknown error'}`;
      return;
    }
    let msg = `Import complete: ${data.imported} imported, ${data.skipped} skipped, ${data.rejected} rejected.`;
    if (data.errors && data.errors.length) {
      msg += '\nRejected rows:\n' + data.errors.map(e => `• Line ${e.line}: ${e.reason}`).join('\n');
    }
